...
# fit gbc
gbc = GradientBoostingClassifier(n_estimators=400, max_depth=3, max_features=3, learning_rate=0.1, random_state=None)
gbc.fit(X_train, y_train.values.ravel())

# data analysis
# extract feature importances
importances = pd.DataFrame(gbc.feature_importances_, index=cols, columns=['Variables'])

# extract partial feature dependence 
features = list(range(1,len(featcols)))  #[1,2,3,4,5,6,7]
fig, axs = plot_partial_dependence(gbc, X, features, feature_names=cols)
fig.set_size_inches(9, 10)
plt.show()

yhatgbc = gbc.predict(X_test)
yhat_traingbc = gbc.predict(X_train)
dsp = 250

## train plot
#plt.plot(y_train.index, y_train.values)
#plt.plot(y_train.index, yhat_traingbc)
#plt.show()

#yhatgbc = bin_classify(yhatgbc,0.3)

# test plot
plt.plot(y_test.index[0:dsp], y_test.values[0:dsp])
plt.plot(y_test.index[0:dsp], yhatgbc[0:dsp], alpha=0.8)
plt.show()

print ("Accuracy of correctly predicting", n_lead,"month fwd = ",'%.1f' % bin_accuracy(yhatgbc, y_test.values, 0.5),'%')
print ("Accuracy of recession predicting", n_lead,"month fwd = ",'%.1f' % class_accuracy(yhatgbc, y_test.values, 0.5),'%')
